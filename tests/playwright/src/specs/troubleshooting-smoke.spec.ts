...
test.describe.serial('Troubleshooting page verification', { tag: '@smoke' }, () => {
  ...
  test('Can reconnect providers', async () => {
    ...
    const status = await troubleshootingPage.reconnetProviders();
  ...
  test('Content of the application Log', async () => {
    ...
    const logList = logs.getByRole('listitem');
    await playExpect(logList).toContainText(
      [
      ...
      { useInnerText: true, ignoreCase: true },
    );
  });
